...
engine = create_engine('mysql://root:admin@localhost:3306/injury', echo=True)
...
def send_notification(sender):
    ...
    # storing to database
    db.execute(text("INSERT INTO gov_message (msg,receiver) VALUES (:s1,:s2)"),
                    {"s1":msg,"s2":receiver})
    db.commit()
    if(receiver=='player'):
        raw = db.execute(text("SELECT number FROM player")).fetchall()
    else:
        raw = db.execute(text("SELECT number FROM applicant")).fetchall()
    ...
    applicant = db.execute(text("SELECT * FROM applicant where status='n'")).fetchall
...
def register():
    ...
    if request.method == 'POST':
        ...
        if user_field=='student':
            ...
            if db.execute(text("SELECT number FROM student WHERE number = :stu_no", {"stu_no": number})).rowcount == 0:
                db.execute(text("INSERT INTO student (name,sex,dob,email,number,aadhar,pincode,qualification,game,password) VALUES (:s1,:s2,:s3,:s4,:s5,:s6,:s7,:s8,:s9,:s10)"),
            ...
            password = str(request.form.get('password'))
            if db.execute(text("SELECT number FROM player WHERE number = :ply_no"), {"ply_no": number}).rowcount == 0:
                db.execute(text("INSERT INTO player (name,sex,dob,email,number,aadhar,pincode,game,level,achievement,job,password) VALUES (:s1,:s2,:s3,:s4,:s5,:s6,:s7,:s8,:s9,:s10,:s11,:s12)"),
            ...
            password = str(request.form.get('password'))
            if db.execute(text("SELECT number FROM student WHERE number = :app_no").bindparams(app_no=number)).rowcount == 0:
                db.execute(
                    text("INSERT INTO applicant (name,sex,dob,email,number,aadhar,pincode,qualification,area,experience,achievement,password) VALUES (:s1,:s2,:s3,:s4,:s5,:s6,:s7,:s8,:s9,:s10,:s11,:s12)").bindparams(
                        s1=name, s2=sex, s3=dob, s4=email, s5=number, s6=aadhar, s7=pincode, s8=qualification, s9=area, s10=experience, s11=achievement, s12=password
                    )
                )
                db.commit()
                return render_template('login-system/signin.html', msg='Account created successfully. Sign in now!')
            else:
                return render_template('login-system/signin.html', msg="Your account exists. Sign in now!")
            ...
            pincode = int(request.form.get('pincode'))
            password = str(request.form.get('password'))
            if db.execute(text("SELECT number FROM goverment WHERE number = :gov_no", {"gov_no": number})).rowcount == 0:
                db.execute(text("INSERT INTO goverment (name,email,number,aadhar,pincode,password) VALUES (:s1,:s2,:s3,:s4,:s5,:s6)"),
...
def verify_signin():
    if request.method == 'POST':
        ...
        if field=='student':
            ...
            if db.execute(text("SELECT number FROM player WHERE number = :number").bindparams(number=number)).rowcount == 0:
                return render_template('login-system/signup.html',msg="Enter mobile number not exist, Register first!")
            else:
                raw_password = db.execute(text("SELECT password FROM player WHERE number = :number"), {"number": number}).scalar()
                password_db = raw_password
                print(password_db)
                print(raw_password)
                if(password == password_db):
                    player = db.execute(text("SELECT * FROM player WHERE number = :ply_no"), {"ply_no": number}).fetchone()
                    msg = db.execute(text("SELECT msg FROM gov_message WHERE receiver = :name"), {"name": 'player'}).fetchall()
                    count = db.execute(text("SELECT count(msg) FROM gov_message WHERE receiver = :name"), {"name": 'player'}).fetchall()
                    return render_template('dashboards/dash_player.html', player=player, msg=msg, count=count[0])
                else:
                    return render_template("login-system/signin.html",msg="Your enter incorrect password, Try again!")

        elif field == 'applicant':
            if db.execute(select([func.count()]).where(applicant.c.number == number)).scalar() == 0:
                return render_template('login-system/signup.html', msg="Enter mobile number not exist, Register first!")
            else:
                raw_password = db.execute(select([applicant.c.password]).where(applicant.c.number == number)).fetchone()
                password_db = raw_password
                if password == password_db:
                    applicant = db.execute(select([applicant]).where(applicant.c.number == number)).fetchone()
                    msg = db.execute(select([gov_message.c.msg]).where(gov_message.c.receiver == 'applicant')).fetchall()
                    count = db.execute(select([func.count()]).where(gov_message.c.receiver == 'applicant')).fetchall()
                    return render_template('dashboards/dash_applicant.html', applicant=applicant, msg=msg, count=count[0][0])
                else:
                    return render_template('login-system/signin.html', msg="Your enter incorrect password, Try again!")
        
        elif field=='goverment':
            if db.execute(text("SELECT number FROM goverment WHERE number = :number", {"number": number})).rowcount == 0:
                return render_template('login-system/signup.html',msg="Enter mobile number not exist, Register first!")
            else:
                raw = db.execute(text("SELECT password,name FROM goverment WHERE number = :number", {"number": number})).fetchone()
                password_db = raw[0]
                name = raw[1]
                if(password == password_db):
                    applicant = db.execute(text("SELECT * FROM applicant WHERE status = 'n'")).fetchall()
...
def gov_action(id):
    if request.method == 'POST':
        ...
        if(act=='accept'):
            db.execute(text("UPDATE applicant SET status = 'y' WHERE applicant_id = :id"), {"id": id})
            db.commit()
            no = db.execute(text("SELECT number FROM applicant where applicant_id = :id"), {"id": id})
            URL = 'http://www.way2sms.com/api/v1/sendCampaign'
